...
import ssi_fc_data.backtest_utils as backtest_utils
import ssi_fc_data.indicators_cal as indicators_cal
...
class Lbmom:
    ...
    def run_simulation(self, historical_data):
        ...
        portfolio_df.loc[0, "capital"] = 100000000
        is_halted = lambda inst, date: not np.isnan(historical_data.loc[date, "{} active".format(inst)]) and (~historical_data[:date].tail(3)["{} active".format(inst)]).any() and (historical_data.loc[date, "{} close".format(inst)] == 0)
        ...
        for i in portfolio_df.index:
            ...
            for inst in non_tradable:
                ...
                portfolio_df.loc[i, "{} units_adj".format(inst)] = 0
            ...
            for inst in tradable:
                ...
                if (3<i) and ((portfolio_df.loc[i-1, f"{inst} units"] - portfolio_df.loc[i, f"{inst} units"]) > portfolio_df.loc[i-2, f"{inst} units"]):
                    portfolio_df.loc[i, f"{inst} units_adj"] = portfolio_df.loc[i-1, f"{inst} units"] - portfolio_df.loc[i-2, f"{inst} units"]
                elif (3<i) and ((portfolio_df.loc[i-2, f"{inst} units"] - portfolio_df.loc[i, f"{inst} units"]) > portfolio_df.loc[i-3, f"{inst} units"]):
                    portfolio_df.loc[i, f"{inst} units_adj"] = portfolio_df.loc[i-2, f"{inst} units"] - portfolio_df.loc[i-3, f"{inst} units"]               
                else:    
                    portfolio_df.loc[i, f"{inst} units_adj"] = portfolio_df.loc[i, f"{inst} units"]
                nominal_total += abs(portfolio_df.loc[i, f"{inst} units_adj"] * inst_price)
            ...
            for inst in tradable:
                units = portfolio_df.loc[i, "{} units_adj".format(inst)]
        ...
        portfolio_df.to_excel(r"C:/Users/luong/Desktop/SSI_API/SSI/Data/lbmom_vn30.xlsx")
        portfolio_df.to_csv(r"C:/Users/luong/Desktop/SSI_API/SSI/Data/lbmom_vn30.csv")
